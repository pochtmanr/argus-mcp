...
The prebuilt workflows that ship with Argus, grouped into packs - each entry's `category` is the pack it belongs to and `vertical` says what that pack is for. The catalogue is deliberately short and hand-picked rather than a store, so READ THE REPLY rather than assuming a recipe for something exists. Every one of them is an AUTOMATION that acts as the user: it drives a real browser in one of this workspace's profiles, or sends from one of its own mailboxes. Each entry gives its slug, its pack and vertical, the site it targets if it targets one named service, the tables it reads and writes, its parameters, any caution it carries about money or mail that cannot be recalled, whether it needs a signed-in profile, whether its selectors have been verified against a captured page, and whether this workspace has it set up or has it in Trash. Read this BEFORE building a workflow with argus_create_automation: if one already does the job, argus_set_up_recipe is one call instead of twelve steps. SCRAPERS ARE NOT HERE and are not automations: a scraper is input in, rows out and no browser, it lives on its own tab with its own record, and it costs none of the plan's automation slots. argus_list_scrapers is that catalogue and argus_run_scraper runs one.
...
The prebuilt collectors that ship with Argus, one card per JOB rather than per service - a service usually has several (its profiles, its posts, the people who follow one). A SCRAPER is not an automation: it reads pages and endpoints it does not own, over http, with NO BROWSER at all, and its whole output is rows in a dataset. It costs none of the plan's automation slots. Most cards run signed out, which is the settled position and the default; a few targets have no signed-out source at all, and those cards need a session that is connected once by hand in the app - a run or a sample started without it stops before its first request and says what to connect, so nothing is ever collected half-signed-in. Read this BEFORE building a workflow with argus_create_automation whenever the ask is 'collect X from Y' - if a card already covers the site, argus_run_scraper is one call instead of twelve steps. Each entry gives its kind (the id every other scraper tool takes), its category, what it collects from, how many columns the table will have, and how this workspace has done with it so far. Do not assume a kind: the catalogue grows, and this call is the only current list of it. For workflows that operate the user's OWN accounts, see argus_list_recipes instead.
...
A dry run of one scraper: one page each of the first few searches it planned, no contact enrichment, and NOTHING WRITTEN - no dataset, no run record, and the handful of sample rows it returns exist only in this reply. What it answers is which columns actually came back with values, which is how an http collector fails: not with an error, but with a table of half-blank rows nobody notices until after a full run. Do this before argus_run_scraper whenever any answer is a guess.
...
Run a scraper for real: collect, filter, optionally visit each row's own site for contact details, then write everything into a NEW dataset in the scraper's folder. Returns a run id IMMEDIATELY and the run continues in the background - it takes minutes and makes a real request per page, so poll argus_scraper_runs to learn how it ended. THE DESTINATION IS ALWAYS A NEW TABLE, one per run, and this tool takes no target: nothing is overwritten and two searches never mix. Appending to, or upserting into, a table you name is offered ONLY by the runScraper automation step - its mode/target/matchOn fields, which argus_automation_schema describes - so a collection that has to accumulate in one table across runs belongs in an automation rather than here. Call argus_sample_scraper first when the answers are a guess. A card that needs a signed-in session refuses before its first request with a message naming what to connect in the app.
